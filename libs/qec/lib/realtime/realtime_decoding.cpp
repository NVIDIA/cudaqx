...
void maybe_init_realtime_session() {
  if (!realtime_mode_inproc_rpc_requested()) {
    CUDAQX_INFO("CUDAQ_QEC_REALTIME_MODE not set to inproc_rpc; using "
  ...
  if (device_mode) {
    ...
    CUDAQX_INFO("CUDAQ_QEC_REALTIME_MODE=inproc_rpc with CPU (non-graph) "
...
void maybe_finalize_realtime_session() {
  if (g_realtime_session) {
    try {
      ...
      CUDAQX_WARN("qec_realtime_session::finalize threw: {}", e.what());
    }
    g_realtime_session.reset();
  }
  if (g_realtime_session_owns_shared_ring_mode) {
...
int configure_decoders(
  ...
  CUDAQX_INFO("Initializing decoders...");
  ...
  for (auto &decoder_config : decoder_configs) {
    if (decoder_ids.count(decoder_config.id) > 0) {
      CUDAQX_WARN("Duplicate decoder ID found: {}", decoder_config.id);
      return 1;
    }
  ...
  if (min_decoder_id < 0) {
    CUDAQX_WARN("Minimum decoder ID is less than 0: {}", min_decoder_id);
    return 3;
  }
  ...
  if (realtime_mode_inproc_rpc_requested()) {
    ...
    if (flags_err != cudaSuccess && flags_err != cudaErrorSetOnActiveProcess)
      CUDAQX_WARN("cudaSetDeviceFlags(cudaDeviceMapHost) returned '{}' before "
  ...
  try {
    ...
    for (const auto &decoder_config : decoder_configs) {
      ...
      CUDAQX_INFO("Creating decoder {} of type {}", decoder_config.id,
                  decoder_config.type);
    ...
    CUDAQX_WARN("Error initializing decoders: {}", e.what());
    return 4;
  }
...
void finalize_decoders() {
  CUDAQX_INFO("Finalizing the realtime decoding library.");
...
void enqueue_syndromes(std::size_t decoder_id, uint8_t *syndromes,
  ...
  CUDAQX_INFO("[decoder={}][tag={}] enqueue_syndrome took {:.3f} us, "
...
void get_corrections(std::size_t decoder_id, uint8_t *corrections,
  ...
  CUDAQX_INFO("Entered get_corrections function decoder_id={}, "
...
void reset_decoder(std::size_t decoder_id) {
  CUDAQX_INFO("Entered reset_decoder for decoder_id={}", decoder_id);
  if (decoder_id >= g_decoders.size()) {
    throw std::invalid_argument(
        fmt::format("Decoder {} not found", decoder_id));
  }
