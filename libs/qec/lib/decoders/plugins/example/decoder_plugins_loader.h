...
public:
    ...
    for (const auto& entry : fs::directory_iterator(directory)) {
        ...
        if (entry.path().extension() == ".so") {
            ...
            SymbolListFunction get_symbols = reinterpret_cast<SymbolListFunction>(dlsym(handle, "get_exported_symbols"));
            const char* dlsym_error = dlerror();
            if (dlsym_error) {
                std::cerr << "dlsym failed to get 'get_exported_symbols': " << dlsym_error << "\n";
                dlclose(handle);
                continue;
            }
            ...
            for (const auto& symbol : symbol_list) {
                ...
                CreatorFunction creator = reinterpret_cast<CreatorFunction>(dlsym(handle, symbol.c_str()));
                const char* dlsym_error = dlerror();
                if (!dlsym_error) {
                    ...
                    std::cerr << "dlsym failed to get symbol' " << symbol << "': " << dlsym_error << "\n";
                }
              }
          }
        }
     ...
     * This function creates a decoder by invoking the creator function of the specified plugin.
     ...
     */
    std::unique_ptr<cudaq::qec::decoder> create_decoder(const std::string& plugin_name, 
