---
name: "cudaq-qec-realtime"
title: "CUDA-Q Libraries QEC Real-Time Decoding (on hardware and in simulation)"
description: >-
  Deploy and run latency-bounded QEC decoding: in-kernel decoder API
  (reset_decoder, enqueue_syndromes, get_corrections), decoder_config /
  multi_decoder_config YAML, configure_decoders_from_file, in-kernel
  sliding-window dispatch, autonomous GPU-resident decoders
  (autonomous_decoder CRTP), the hybrid AI predecoder + PyMatching
  pipeline (AIDecoderService, AIPreDecoderService), wiring a
  trt_decoder in-kernel, FPGA ring-buffer dispatch, CUDA Graphs,
  NVTX instrumentation, and real-time decoding on Quantinuum Helios.
  Use whenever the user mentions real-time decoding, in-kernel
  decoding, autonomous_decoder, AIDecoderService, AIPreDecoderService,
  CUDA Graph QEC, Helios, Quantinuum, FPGA, ring buffer, decoder
  dispatch, RPC, "low latency QEC", or "deploy a trained AI
  predecoder". Do NOT use this skill for: training neural decoders
  (use cudaq-qec-ai-decoders); offline / code-capacity /
  DEM-sampling experiments (use cudaq-qec-decode); profiling
  existing code for performance regressions (use
...
tags: [cudaq, cudaq-qec, real-time, in-kernel, autonomous-decoder, cuda-graph, helios, quantinuum, fpga, predecoder]
...
# CUDA-Q Libraries Real-Time Decoding
...
sub-millisecond budgets. The base QEC skill (`cudaq-qec-decode`) covers
...
delegate to `cudaq-qec-decode`. If they are training an ML decoder, hand
off to `cudaq-qec-ai-decoders`. This skill assumes the model is fixed
...
new code/decoder plugins (→ `cudaq-qec-extending`).
...
| Triage suspicious corrections, hangs, config issues, or latency | `references/realtime-triage.md` |
...
[ ] If corrections, config loading, hardware behavior, or latency are suspicious:
    checked references/realtime-triage.md.
```
...
7. For suspicious corrections, hangs, configuration loading, hardware
   behavior, or latency, read `references/realtime-triage.md`.
...
- Workflow references: `references/in-kernel.md`,
  ...
  `references/hardware-helios.md`, `references/realtime-triage.md`.
- Base QEC workflows (offline decoding, custom codes):
  `cudaq-qec-decode/SKILL.md`.
- Training neural decoders and ONNX → TensorRT:
  `cudaq-qec-ai-decoders/SKILL.md`.
- GPU profiling, NVTX, `nsys`: `cudaq-profiling-perf/SKILL.md`.
