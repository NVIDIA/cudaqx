...
fastest way to try CUDA-QX. Both ship Python wheels for Linux
...
CUDA-QX ships separate wheels for CUDA 12 (`-cu12`) and CUDA 13
...
You can run cudaqx on a CPU-only system. What works and what doesn't:
...
```bash
git clone --depth 1 https://github.com/NVIDIA/cudaqx.git
cd cudaqx
...
If the example runs, delegate to `cuda-qx-qec-decode` for deeper QEC work
or `cuda-qx-solvers-algorithms` for variational algorithms.
...
Point them at the published docs <https://nvidia.github.io/cudaqx/>
