# Triage: "the LER looks wrong" + decoder pitfalls
...
## Serving the rendered docs offline

For browsing the cudaq QEC API offline (the rendered HTML lives under
`build/docs/sphinx/` after `bash scripts/build_docs.sh`):

```bash
python -m http.server -d build/docs/sphinx 8080
# then open http://localhost:8080
```

Relative links and search work only when served, not when opened from
`file://`.
...
`osd_order=k`. Other tuning parameters: `max_iterations`, `use_sparsity`,
...
For suspicious convergence, iteration, gamma, BP/OSD, or latency behavior,
read `nv-qldpc-bp-osd-triage.md`. Keep that discussion public-safe:
API-level metrics and parameter relationships only, not private
...
[ ] If nv-qldpc-decoder convergence, iterations, or latency are suspicious:
    checked nv-qldpc-bp-osd-triage.md.
```
