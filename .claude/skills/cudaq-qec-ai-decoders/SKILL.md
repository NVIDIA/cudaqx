---
name: "cudaq-qec-ai-decoders"
title: "CUDA-Q Libraries QEC AI Decoders (training, ONNX, TensorRT)"
description: >-
  Train, export, and build neural-network decoders for QEC: dataset
  generation from Stim circuits, MLP / transformer training in
  PyTorch, ONNX export, and TensorRT engine build
  (scripts/build_engine_from_onnx.py). Use whenever the user mentions
  training a decoder, neural decoder, MLP decoder, transformer
  decoder, ONNX export, TensorRT engine build, build_engine_from_onnx,
  or "training data for a decoder". Do NOT use this skill for:
  deploying a trained engine in-kernel, AIDecoderService /
  AIPreDecoderService runtime, or wiring trt_decoder into a real-time
  pipeline — those go to cudaq-qec-realtime. For loading a
  trt_decoder in an offline DEM-sampling experiment, use
  cudaq-qec-decode. For profiling the training loop itself, use
  cudaq-profiling-perf.
version: "0.1.1"
author: "CUDA-Q Libraries"
license: "Apache License 2.0"
compatibility: "Python 3.11+, PyTorch 2.0+, TensorRT (matching CUDA), NVIDIA GPU"
tags: [cudaq, cudaq-qec, ai-decoder, machine-learning, pytorch, onnx, tensorrt, trt-decoder, hybrid-predecoder, mlp]
...
`cudaq-qec-realtime` (`ai-predecoder-pipeline.md`). This skill
...
`cudaq-qec-realtime`); a registered decoder plugin (→
`cudaq-qec-extending`).
...
| Triage suspicious AI decoder accuracy, export, engine, or integration behavior | `references/ai-decoder-triage.md` |
...
[ ] If accuracy, export, engine, or integration behavior is suspicious:
    checked references/ai-decoder-triage.md.
```
...
3. Profile with `nsys` (see `cudaq-profiling-perf`): if engine
...
5. For suspicious accuracy, export, engine, or integration behavior,
   read `references/ai-decoder-triage.md`.
...
- `references/hybrid-deployment.md` — drop into the predecoder +
  PyMatching pipeline.
- `references/ai-decoder-triage.md` — public-safe triage for
  suspicious AI decoder accuracy, export, engine, or integration
  behavior.
- Real-time deployment: `cudaq-qec-realtime` SKILL.md.
- Benchmarking the decoder: `cudaq-benchmarking` SKILL.md.
- Profiling: `cudaq-profiling-perf` SKILL.md.
