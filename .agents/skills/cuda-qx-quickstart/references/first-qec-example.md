...
|----------------------|------|
| Run circuit-level noise | `docs/sphinx/examples/qec/python/circuit_level_noise.py` and `cuda-qx-qec-decode` SKILL.md |
| Try a stronger decoder | `cuda-qx-qec-decode/references/decode.md` "Decoder Selection" |
| Plot LER vs physical error rate | `pseudo_threshold.py` example and `cuda-qx-benchmarking` SKILL.md |
| Write a new code or decoder | `cuda-qx-qec-extending` SKILL.md |
| Decode in real time on hardware | `cuda-qx-qec-realtime` SKILL.md |

At this point the user has a working QEC stack. Hand them off to
`cuda-qx-qec-decode` for the workflow index.
