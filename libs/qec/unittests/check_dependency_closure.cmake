...
cmake_minimum_required(VERSION 3.28)
        ...
        if (NOT DEFINED LIBS) message(FATAL_ERROR "check_dependency_closure: "
                                                  "LIBS not provided") endif() if (
            NOT DEFINED
                FORBIDDEN) message(FATAL_ERROR
                                   "check_dependency_closure: FORBIDDEN not "
                                   "provided") endif() if (NOT DEFINED
                                                               TRANSITIVE) set(TRANSITIVE
                                                                                   ON) endif()

            find_program(READELF_EXECUTABLE NAMES readelf llvm - readelf) if (
                NOT READELF_EXECUTABLE) message(FATAL_ERROR "check_dependency_"
                                                            "closure: neither "
                                                            "readelf nor "
                                                            "llvm-readelf was "
                                                            "found") endif()
...
#separate arguments.
                string(REPLACE "|"
                    ...
                    set(_extra_search_dirs "") if (
                        DEFINED EXTRA_SEARCH_DIRS) string(REPLACE
                                                          "|"
                                                          ";" _extra_search_dirs
                                                          "${EXTRA_SEARCH_"
                                                          "DIRS}") endif()
...
#name that merely shares the prefix.
                        set(_forbidden_regexes "") foreach (
                            _stem IN LISTS
                                _forbidden_stems) string(REGEX REPLACE
                                                         "([][+.*()^$?|\\])"
                                                         "\\\\\\1" _escaped
                                                         "${_stem}")
                            list(APPEND _forbidden_regexes
                                 "^${_escaped}\\.so") endforeach()

#Parse `readelf - d < lib>` output into the list of DT_NEEDED sonames and the
#list of DT_RUNPATH / DT_RPATH search directories.
                                function(read_elf_dynamic lib out_needed
                                             out_rpath) set(_needed "")
                                    set(_rpath "") execute_process(
                                        COMMAND "${READELF_EXECUTABLE}" - d
                                        "${lib}" OUTPUT_VARIABLE _out
                                            ERROR_VARIABLE _err RESULT_VARIABLE
                                                _rc) if (NOT _rc EQUAL 0)
                                        message(
                                            WARNING
                                            "check_dependency_closure: readelf "
                                            "failed on ${lib}: ${_err}") set($ {
                                          out_needed
                                        } "" PARENT_SCOPE) set($ {
                                          out_rpath
                                            ...
                                            string(REGEX MATCHALL "[^\n]"
                                                                  "+" _lines "$"
                                                                             ...
                                                                             "}") foreach (
                                                _line IN LISTS
                                                    _lines) if (_line MATCHES
                                                                "\\(NEEDED\\).*"
                                                                "\\[(.+)\\]")
                                                list(APPEND _needed "${CMAKE_"
                                                                    "MATCH_1}") elseif(
                                                    _line MATCHES
                                                    "\\((RUNPATH|RPATH)\\).*\\["
                                                    "(.+)\\]") string(REPLACE
...
#order.Returns an empty string when the soname cannot be located.
        function(resolve_soname soname search_dirs out_path)
            foreach (_dir IN LISTS search_dirs) if (EXISTS "${_dir}/${soname}")
...
#(e.g.an $ORIGIN /..rpath vs an absolute dir) dedupes in `_visited`.
                get_filename_component(_real "${_dir}/${soname}" REALPATH)
                    set($ { out_path } "${_real}" PARENT_SCOPE) return ()
                        endif() endforeach() set($ { out_path } "" PARENT_SCOPE)
                            endfunction() #end
    -
    resolve_soname()
...
#that led to each queued library(used for diagnostics).
        set(_visited "") set(_queue "") set(_chains "") set(
            _input_names
            "") foreach (_lib IN LISTS _input_libs) if (NOT EXISTS "${_lib}")
            message(FATAL_ERROR "check_dependency_closure: library not found: "
                                "${_lib}") endif() get_filename_component(
                _lib "${_lib}" REALPATH) list(APPEND _queue "${_lib}")
                get_filename_component(_libname "${_lib}" NAME) list(
                    APPEND _chains
                    "${_libname}") list(APPEND _input_names
                                        "${_libname}") endforeach()

                    list(LENGTH _input_libs _input_count) string(
                        REPLACE
                        ";"
                        ", " _forbidden_display
                        "${_forbidden_stems}") message(STATUS
                                                       "check_dependency_"
                                                       "closure: inspecting "
                                                       "${_input_count} "
                                                       "library(ies) for "
                                                       ...
                                                       "[${_forbidden_display}]"
                                                       ":")
                        foreach (_name IN LISTS _input_names) message(
                            STATUS "    - ${_name}") endforeach()

                            set(_violations "")

                                while (_queue) list(POP_FRONT _queue _lib) list(
                                    POP_FRONT _chains _chain)

                                    if (_lib IN_LIST _visited) continue() endif() list(
                                        APPEND _visited
                                        "${_lib}")

                                        read_elf_dynamic("${_lib}" _needed _rpath) get_filename_component(
                                            _libdir
                                            "${_lib}" DIRECTORY)
...
#then its(expanded) RUNPATH / RPATH entries, then any caller - provided extras.
                                            set(_search "${_libdir}") foreach (
                                                _r IN
                                                    LISTS _rpath) string(REPLACE
                                                                         "$ORIG"
                                                                         "IN"
                                                                         "${_"
                                                                         "libdi"
                                                ...
                                                string(
                                                    ...
                                                    "\${ORIGIN}"
                                                    "${_libdir}" _r
                                                    "${_r}") list(APPEND _search
                                                                  "${_r}") endforeach() list(APPEND _search ${
                                                    _extra_search_dirs})

                                                    foreach (_soname IN LISTS _needed) foreach (_re IN LISTS _forbidden_regexes) if (
                                                        _soname
                                                            MATCHES
                                                        "${_re}") list(APPEND _violations
                                                                       "${_"
                                                                       "chain} "
                                                                       "-> "
                                                                       "${_"
                                                        ...
                                                        if (TRANSITIVE)
                                                            resolve_soname(
                                                                ...
                                                                "${_"
                                                                "search"
                                                                "}" _resolved) if (_resolved AND NOT _resolved IN_LIST
                                                                                       _visited) list(APPEND
                                                                                                          _queue "${_resolved}")
                                                                list(APPEND
                                                                         _chains
                                                                     "${_chain}"
                                                                     " -> "
                                                                     "${_"
                                                                     "soname}") endif() endif() endforeach() endwhile()
...
#condition(negative self - test); otherwise any violation is a hard failure.
                                                                        if (EXPECT_FAIL) if (_violations) message(
                                                                            STATUS
                                                                            "ch"
                                                                            "ec"
                                                                            "k_"
                                                                            ...
                                                                            ".") return ()
                                                                                ...
                                                                                "dependency was found in the closure of: ${_input_libs}")
                                                                                    ...
                                                                                    if (_violations) set(
                                                                                        _msg
                                                                                        "check_dependency_closure: forbidden CUDA-Q libraries found in the dependency closure:\n")
                                                                                        foreach (
                                                                                            _v IN LISTS
                                                                                                _violations)
                                                                                            string(
...
#was walked to reach that verdict.
                                                                                                            string(
                                                                                                                REPLACE ";"
                                                                                                                        ", " _input_display "${_input_names}") list(LENGTH _visited _visited_count) message(STATUS "check_dependency_closure: OK -- no forbidden CUDA-Q libraries "
                                                                                                                                                                                                                   "[${_forbidden_display}] in the closure of: ${_input_display}") message(STATUS "check_dependency_closure: ${_visited_count} library(ies) walked in the "
                                                                                                                                                                                                                                                                                                  "closure:") set(_closure_names
                                                                                                                                                                                                                                                                                                                  "") foreach (_lib IN
                                                                                                                                                                                                                                                                                                                                   LISTS
                                                                                                                                                                                                                                                                                                                                       _visited)
