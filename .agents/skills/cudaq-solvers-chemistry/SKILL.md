---
name: "cudaq-solvers-chemistry"
title: "CUDA-Q Libraries Solvers — Quantum Chemistry"
description: >-
  Quantum chemistry deep-dive for CUDA-Q Libraries: molecule construction with
...
tags: [cudaq, cudaq-solvers, chemistry, pyscf, molecule, basis-set, active-space, fermion-to-qubit, operator-pool, classical-baselines]
...
# CUDA-Q Libraries Quantum Chemistry
...
delegate to `cudaq-solvers-algorithms/references/qaoa.md`. If they are choosing
an algorithm (VQE vs ADAPT vs GQE), delegate to `cudaq-solvers-algorithms`. This
...
Does NOT produce: VQE/ADAPT/QAOA/GQE convergence (→ `cudaq-solvers-algorithms`);
custom operator pools / optimizers (→ `cudaq-solvers-extending`).
...
6. **`libgfortran` is a runtime dependency** (for `cobyla` / `lbfgs`).
   Quick fix: `apt install gfortran`. See `cudaq-build` for the full
   explanation.
...
3. Reproduce on H2 first — if H2 works and your molecule doesn't,
   it's a basis / active-space / mapping issue, not a CUDA-Q Libraries issue.
...
- Variational algorithms (VQE, ADAPT, GQE): `cudaq-solvers-algorithms/SKILL.md`.
- Benchmarking energies: `cudaq-benchmarking` SKILL.md.
