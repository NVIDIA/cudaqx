...
# ============================================================================ #
"""Generate D_sparse.txt for surface_code_ising_realtime_decoding's trt+Ising
path.

The Ising decoding project (https://github.com/NVIDIA/Ising-Decoding) provides a
pretrained surface-code predecoder, and its generate_test_data.py exports
H_csr.bin/O_csr.bin/priors.bin in Ising detector order. Because
surface_code_ising_realtime_decoding reads the cudaqx live measurement buffer,
D_sparse.txt bridges the two: one row per Ising detector, whose entries are
cudaqx live-buffer measurement indices, so each row reproduces one cudaqx
detector bit in Ising's detector row order. The cudaqx live measurement stream
then feeds Ising's H/O/priors.

Recipe (see the realtime_decoding_demo documentation for the full flow):
  1. In the Ising repo: generate_test_data.py --distance D --n-rounds T \
       --basis Z --code-rotation XV   -> H_csr.bin/O_csr.bin/priors.bin
  2. Run surface_code_ising_realtime_decoding --save_dem ... once; it prints
     cnot_schedX_flat / cnot_schedZ_flat lines. Save that stdout to sched.txt.
  3. python gen_dsparse_from_memory_circuit.py D T Z XV sched.txt \
       <artifacts>/D_sparse.txt --ising-repo /path/to/Ising-Decoding/code
  4. Run surface_code_ising_realtime_decoding --ising-artifacts-dir \
       <artifacts> ...
...
def parse_args(argv):
    ap = argparse.ArgumentParser(
        description="Generate D_sparse.txt for "
        "surface_code_ising_realtime_decoding's trt+Ising path.")
