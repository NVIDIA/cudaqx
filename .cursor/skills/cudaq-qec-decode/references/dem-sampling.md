...
"head-to-head" comparison pattern in `cudaq-benchmarking`),
...
- **PyTorch CUDA tensors**: accepted when `torch` is installed by the
  user (`pip install torch`). The cudaq wheel does not bundle torch.
...
- Compare decoders on the same shots:
  `cudaq-benchmarking/references/decoder-comparison.md`.
...
- Wrap the inner decoder in a sliding window for streaming:
  `cudaq-qec-realtime/references/sliding-window.md`.
