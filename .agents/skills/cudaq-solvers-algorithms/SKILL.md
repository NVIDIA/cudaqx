---
name: "cudaq-solvers-algorithms"
title: "CUDA-Q Libraries Solvers — Algorithm Dispatch"
description: >-
  ...
  `cudaq-solvers-chemistry`); writing a new operator pool / optimizer
  (use `cudaq-solvers-extending`).
version: "0.3.3"
author: "CUDA-Q Libraries"
license: "Apache License 2.0"
compatibility: "Python 3.11+, C++ 20, Linux x86_64/aarch64"
tags: [cudaq, solvers, chemistry, optimization, vqe, adapt-vqe, qaoa, gqe]
...
- A qubit Hamiltonian (`cudaq.SpinOperator`) — typically from
  `cudaq-solvers-chemistry`'s output, or constructed directly.
...
Does NOT produce: the molecule itself (→ `cudaq-solvers-chemistry`);
new operator pools / optimizers (→ `cudaq-solvers-extending`).
...
basis sets, classical baselines), delegate to **`cudaq-solvers-chemistry`**.
...
For `chemistry` intent, dispatch to `cudaq-solvers-chemistry`.
...
`cudaq-solvers-chemistry`. Plugin authoring (custom pools /
optimizers) belongs to `cudaq-solvers-extending`.
...
| Molecule, operator pools, fermion-to-qubit mappings (PySCF)     | `cudaq-solvers-chemistry`                |
| Custom operator pool / optimizer / state-prep                   | `cudaq-solvers-extending`                |
...
- Useful environment variables (set before `import cudaq_solvers`):
  - `OMP_NUM_THREADS=1` — chemistry reproducibility; see
    `cudaq-solvers-chemistry` for why.
...
PySCF server), see `cudaq-solvers-chemistry` SKILL.md.
...
4. **`libgfortran` is a runtime dependency** for `cobyla` and `lbfgs`.
   ...
   `cudaq-build` (Convention #3). Linux x86_64/aarch64 only.
...
- **`cudaq-solvers-chemistry`** — PySCF, molecule construction, active space,
  operator pools, fermion-to-qubit mappings, classical baselines.
- **`cudaq-solvers-extending`** — write a new operator pool, optimizer, or
  state-prep kernel.
- **`cudaq-benchmarking`** — VQE / ADAPT / GQE energy and time-to-
  chemical-accuracy comparisons.
- **`cudaq-profiling-perf`** — `nsys` / `ncu` for slow VQE / GQE loops.
