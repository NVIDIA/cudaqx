...
void enqueue_syndromes_ui64(std::uint64_t decoder_id,
                            ...
                            std::uint64_t tag) {
  CUDAQX_INFO("Entering enqueue_syndromes_ui64 for decoder id: {} and tag: {}",
              decoder_id, tag);
  if (z_use_private_impl) {
...
std::uint64_t get_corrections_ui64(std::uint64_t decoder_id,
                                   ...
                                   std::uint64_t reset) {
  CUDAQX_INFO("Entering get_corrections_ui64 for decoder id: {}", decoder_id);
  if (z_use_private_impl) {
...
void reset_decoder_ui64(std::uint64_t decoder_id) {
  CUDAQX_INFO("Entering reset_decoder_ui64 for decoder id: {}", decoder_id);
  if (z_use_private_impl) {
