---
name: "cudaq-qec-extending"
title: "CUDA-Q Libraries QEC Extending (custom codes and decoders)"
description: >-
  Plugin-author skill for the QEC library: define a new QEC code
  (Python or C++), register a new decoder as a plugin (Python or C++),
  hook into the plugin loader, expose it through cudaq_qec, and add
  unit tests. Use whenever the user mentions "custom QEC code", "new
  QEC code", "register a decoder plugin", "QEC plugin", "subclass code",
  "plugin_loader", or "extend cudaq_qec". Do NOT use for: configuring
  or running an existing decoder (use cudaq-qec-decode); writing a
  new real-time / autonomous_decoder (use cudaq-qec-realtime);
  contribution mechanics, DCO sign-off, or adding a test against an
  existing decoder (use cudaq-contributing);
  solvers extensions (operator pools, optimizers, state-prep — use
  cudaq-solvers-extending).
version: "0.1.2"
...
tags: [cudaq, cudaq-qec, qec, plugin, extension, custom-code, custom-decoder, plugin-loader]
...
# CUDA-Q Libraries QEC Extending
...
to `cudaq-qec-decode`. If they want to write a new real-time / autonomous
decoder (GPU-resident, zero-CPU), delegate to
`cudaq-qec-realtime/references/autonomous-decoder.md` — the
device-side path has its own contract. For solvers operator pools,
state-prep kernels, or optimizers, see the cudaq-solvers-algorithms package
(`cudaq-solvers-extending` skill).
...
`cudaq-qec-realtime/references/autonomous-decoder.md`); a trained
neural decoder (→ `cudaq-qec-ai-decoders`).
...
Q3: Real-time / latency-bounded?
    yes -> C++ + autonomous_decoder. cudaq-qec-realtime/references/autonomous-decoder.md
...
4. For build issues, delegate to `cudaq-build`.
...
- Testing your plugin: `cudaq-contributing` SKILL.md (testing references).
- Real-time eligible decoders: `cudaq-qec-realtime` SKILL.md.
- Solvers extensions (operator pools etc.): see the cudaq-solvers-algorithms
  package — `cudaq-solvers-extending` skill when in the cudaq monorepo.
