...
protected:
    ...
    void SetUp() override {
        std::vector<double> h2_data{
          ...
          cudaq::solvers::molecular_geometry geometryHH = {
            {"H", {0., 0., 0.}},
            {"H", {0., 0., .7474}}
          };
          auto hh = cudaq::solvers::create_molecule(geometryHH, "sto-3g", 0, 0, 
          {.casci = true, .ccsd = true, .verbose = true});
          auto lih = cudaq::solvers::create_molecule(geometryLiH, "sto-3g", 0, 0,
          {.casci = true, .ccsd = true, .verbose = true});
...
TEST_F(SolversTester, checkSimpleAdapt) {
  ...
  auto [energy1, thetash1, ops1] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, h, poolList, {{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy1, -1.13, 1e-2);

  poolList = pool->generate({{"num-orbitals", hamhh.num_qubits() / 2}});
  auto [energy3, thetas3, ops3] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, hamhh, poolList, {{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy3, -1.13, 1e-2);

  poolList = pool->generate({{"num-orbitals", hamli.num_qubits() / 2}});
  auto [energy2, thetas2, ops2] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, hamli, poolList, {{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy2, -7.8473, 1e-2);
...
TEST_F(SolversTester, checkSimpleAdaptGradient) {
  ...
  auto poolList = pool->generate({{"num-orbitals", h.num_qubits() / 2}});
  auto [energy1, thetas1, ops1] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, h, poolList, *opt, "central_difference", {{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy1, -1.13, 1e-2);
  for (std::size_t i = 0; i < thetas1.size(); i++)
    printf("%lf -> %s\n", thetas1[i], ops1[i].to_string().c_str());

  poolList = pool->generate({{"num-orbitals", h.num_qubits() / 2}});
  auto [energy3, thetas3, ops3] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, hamhh, poolList, *opt, "central_difference", {{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy3, -1.13, 1e-2);
  for (std::size_t i = 0; i < thetas3.size(); i++)
    printf("%lf -> %s\n", thetas3[i], ops3[i].to_string().c_str());

  poolList = pool->generate({{"num-orbitals", hamli.num_qubits() / 2}});
  auto [energy2, thetas2, ops2] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, hamli, poolList, *opt, "central_difference", {{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy2, -7.8473, 1e-2);
...
TEST_F(SolversTester, checkSimpleAdaptUCCSD) {
  ...
  auto [energy1, thetas1, ops1] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, h, poolList,{{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
  EXPECT_NEAR(energy1, -1.13, 1e-2);
  ...
  config2.insert("num-electrons", 2);
...
TEST_F(SolversTester, checkSimpleAdaptGradientUCCSD) {
  ...
  config1.insert("num-electrons", 2);
  auto poolList = pool->generate(config1);
  auto [energy, thetas, ops] = cudaq::solvers::adapt_vqe(hartreeFock2Electrons, h, poolList, *opt, "central_difference",{{"grad_norm_tolerance", 1e-3}, {"verbose", true}});
