...
/// @file cancellation.h
/// @brief `cudaq::qec::cancellation_token` / `cudaq::qec::cancellation_source`: a small,
/// header-only, cooperative-cancellation pair.
///
...
/// cancellation receives a token (see `decoder::decode(syndrome, cancellation_token)`
/// in `decoder.h`) and polls it, reacting to the requested level. Cancellation may be
/// `soft` or `hard`.
///
...
/// @brief Requested stop level, ordered by severity. See the file header for
/// how a decoder is expected to react to each level.
enum class cancellation_level : std::uint8_t {
  none = 0, ///< No stop requested.
  soft = 1, ///< Stop only if the current node is marked soft-cancellable.
  hard = 2  ///< Abort as soon as possible.
};

class cancellation_source;

/// @brief Read-only view of a `cancellation_source`'s shared cancellation state.
...
public:
  ...
  /// @brief The currently requested level (`cancellation_level::none` for a
...
/// @brief Owns the cancellation state observed by the `cancellation_token`s obtained
/// from it. The state outlives the source for as long as any token referencing it exists
class cancellation_source {
public:
  cancellation_source()
      : state_(std::make_shared<cancellation_token::state_type>(cancellation_level::none)) {}
  ...
  bool request_soft_stop() noexcept { return request(cancellation_level::soft); }

  /// @brief Request a hard stop (see `cancellation_level::hard`). 
  bool request_hard_stop() noexcept { return request(cancellation_level::hard); }

  cancellation_level level() const noexcept {
    return state_->load(std::memory_order_acquire);
  }

  cancellation_token get_token() const noexcept { return cancellation_token(state_); }
