...
NEVER .claude/skills/<name>/...                 # generated mirror
NEVER .cursor/skills/<name>/...                 # generated mirror
```

Editing a mirror has no effect on tracked content and will be
overwritten the next time `sync_agents_skills.sh` runs. Mirrors are
tracked for clone-ready agent support, but they are never edited by hand.
...
Commit the canonical edit and regenerated `.claude/skills/` /
`.cursor/skills/` mirrors together.
...
bash scripts/sync_agents_skills.sh
git add .agents/skills/<name> .claude/skills/<name> .cursor/skills/<name>
```
...
# Update AGENTS.md
bash scripts/sync_agents_skills.sh
git add .agents/skills/new-name .claude/skills/new-name .cursor/skills/new-name
```
...
- Regenerated `.claude/skills/` and `.cursor/skills/` mirrors are staged.
...
```bash
grep -rl "cudaq-OLDNAME" .agents/ AGENTS.md README.md
```
...
| CI says mirrors stale, you swear you ran sync | uncommitted edit in `.agents/skills/` or unstaged mirror output; `git status` to check |
