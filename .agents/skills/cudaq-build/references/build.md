...
| `CMAKE_INSTALL_PREFIX`       | `$HOME/.cudaq` | Where install/headers/python packages land            |
...
export CUDAQX_INSTALL_PREFIX=${CUDAQX_INSTALL_PREFIX:-$HOME/.cudaq}
...
- `$HOME/.cudaq/include` and `$HOME/.cudaq/lib` populated by `ninja install`.
...
# Drop installed CUDA-Q Libraries.
rm -rf $HOME/.cudaq
