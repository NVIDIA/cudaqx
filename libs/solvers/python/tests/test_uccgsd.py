...
def test_solvers_vqe_uccgsd_h2():
    ...
    def cost(theta):
        ...
        energy = cudaq.observe(ansatz, molecule.hamiltonian, 
                               numQubits, numElectrons, theta, 
                               pauliWordsList, coefficientsList).expectation()
        return energy

    res = minimize(cost, x0, method='COBYLA', options={'maxiter': 1000, 'rhobeg': 0.1, 'disp': True})
    energy = res.fun
...
def test_solvers_vqe_uccgsd_lih():
    ...
    doubles = (numQubits * (numQubits - 1) * (numQubits - 2) * (numQubits - 3)) / 8
    ...
    res = minimize(cost, x0, method='COBYLA', options={'maxiter': 6000, 'rhobeg': 0.1})
    energy = res.fun
