...
Runs inside (or against) the released `ghcr.io/nvidia/cudaq`
container. Verifies:

- CUDA-Q imports and runs.
- CUDA-Q Libraries imports.
...
docker run --gpus all -it ghcr.io/nvidia/cudaq \
  bash /workspaces/cudaq/scripts/validation/container/validate_container.sh
```
...
- Documentation rendering. (Sphinx build is its own CI job; see
  `cudaq-build/references/docs.md`.)
...
- Wheel build pipeline: `cudaq-build/references/wheels.md`.
- Container build pipeline: `cudaq-build/references/build.md` (and
  `docker/` paths).
