...
void sliding_window::validate_inputs() {
  ...
  if (inner_decoder_params.empty()) {
    CUDAQX_WARN("sliding_window constructor: inner_decoder_params is empty. "
                "Is that intentional?");
  }
...
void sliding_window::initialize_window(std::size_t num_syndromes) {
  ...
  CUDAQX_DBG("Initializing window");
...
sliding_window::sliding_window(const cudaq::qec::sparse_binary_matrix &H,
  ...
  for (std::size_t w = 0; w < num_windows; ++w) {
    ...
    CUDAQX_INFO("Creating a decoder for rounds {}-{} (dims {} x {}) "
...
decoder_result sliding_window::decode(const std::vector<float_t> &syndrome) {
  if (syndrome.size() == this->syndrome_size) {
    auto t0 = std::chrono::high_resolution_clock::now();
    CUDAQX_DBG("Decoding whole block");
    ...
    CUDAQX_INFO("Whole block time: {:.3f} ms", diff.count() * 1000);
    return result;
  }
  ...
  if (this->rw_filled == num_syndromes_per_window) {
    auto t0 = std::chrono::high_resolution_clock::now();
    CUDAQX_DBG("Window is full, sliding the window by one round");
    ...
    auto t0 = std::chrono::high_resolution_clock::now();
    CUDAQX_DBG("Copying data to the end of the rolling window");
    add_syndrome_to_rolling_window(syndrome, 0);
  ...
  if (rw_filled == num_syndromes_per_window &&
    ...
    CUDAQX_DBG("Decoding window {}/{}", num_windows_decoded + 1, num_windows);
    decode_window();
    num_rounds_since_last_decode = 0;
    ...
    if (num_windows_decoded == num_windows) {
      ...
      rw_filled = 0;
      // for (std::size_t w = 0; w < num_windows; ++w) {
      //   CUDAQX_DBG("Window {} time: {} ms", w, window_proc_times[w]);
      // }
      CUDAQX_DBG("Returning decoder_result");
      return std::move(this->rw_results[0]);
    }
  }
  CUDAQX_DBG("Returning empty decoder_result");
...
std::vector<decoder_result> sliding_window::decode_batch(
    const std::vector<std::vector<float_t>> &syndromes) {
  if (syndromes.empty()) {
    CUDAQX_DBG("Returning empty decoder_result (no syndrome)");
    return {};
  }
...
void sliding_window::decode_window() {
  ...
  CUDAQX_DBG("Window {}: syndrome_start = {}, syndrome_end = {}, length1 = "
             "{}, length2 = {}",
  ...
  if (!inner_results[0].converged) {
    CUDAQX_DBG("Window {}: inner decoder failed to converge", w);
  }
  ...
  if (w < num_windows - 1) {
    ...
    CUDAQX_DBG("  Committing {} bits from window {}", num_to_commit, w);
    for (std::size_t s = 0; s < this->rolling_window.size(); ++s) {
  ...
  CUDAQX_INFO("Window {} time: {:.3f} ms (0:{:.3f}ms 1:{:.3f}ms 2:{:.3f}ms "
