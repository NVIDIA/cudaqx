...
#
# Python counterpart to surface_code-1.cpp. Structural parity with that file;
...
def decoder_args(decoder_type: str, error_rates: list) -> dict:
    """Return the custom-args dict for a named decoder."""
...
def stabilizer_round(data: cudaq.qview, anc: cudaq.qview, num_ancz: int,
    ...
    return mz(anc)  # ancz first then ancx


@cudaq.kernel
def memory_circuit(data: cudaq.qview, anc: cudaq.qview, num_ancz: int,
                   num_ancx: int, num_rounds: int, x_schedule: List[int],
                   z_schedule: List[int], decoder_id: int,
                   inject_error: int) -> List[cudaq.measure_handle]:
    """Per-logical memory circuit: prep, lock-in, rounds, data readout."""
    prep_0_data(data)

    # Inject a deterministic X error on data qubit 0 when requested
    if inject_error:
        x(data[0])
...
def demo_circuit_qpu(num_data: int, num_ancx: int, num_ancz: int,
                     ...
                     num_obs: int, inject_error: int) -> int:
...
def run_shots(num_shots: int, noise, obs_flat: list, num_obs: int, opts,
    ...
    # On the remote (noiseless) path inject a deterministic error so the
    # decoder is exercised even without an attached noise model.
    inject_error = 1 if is_remote else 0
...
def run(argv: List[str]) -> Optional[dict]:
    ...
    parser.add_argument("--max_cost",
                        type=int,
                        default=-1,
                        help="Max HQC cost for quantinuum QPU runs")
    parser.add_argument("--max_qubits",
                        type=int,
                        default=-1,
                        help="Max qubits for quantinuum QPU runs")
