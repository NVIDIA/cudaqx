---
name: "cuda-qx-benchmarking"
title: "CUDA-QX Benchmarking and Reproducibility"
description: >-
  Run apples-to-apples comparisons for CUDA-QX work: QEC logical-error-
  rate sweeps, pseudo-threshold curves, decoder accuracy vs speed,
  VQE / ADAPT / GQE energy convergence and time-to-solution, throughput
  / tail-latency for real-time decoding. Covers seeding,
  OMP_NUM_THREADS reproducibility, statistical error bars, plotting
  conventions, paper-grade tables, and what to publish vs what to hide
  in supplementary. Use whenever the user mentions benchmarking,
  threshold, pseudo-threshold, scaling, error bars, statistical
  significance, reproducibility, "compare decoder A vs B", "fair
  comparison", paper plots, or "what should I publish".
version: "0.1.1"
author: "CUDA-QX"
license: "Apache License 2.0"
compatibility: "Python 3.11+, Linux x86_64/aarch64 (some GPU-only paths)"
tags: [cuda-qx, benchmarking, reproducibility, threshold, pseudo-threshold, error-bars, statistics, scaling, paper-results]
...
# CUDA-QX Benchmarking and Reproducibility
...
Does NOT produce: the actual decode (→ `cuda-qx-qec-decode`); the actual
variational run (→ `cuda-qx-solvers-algorithms`); profiling traces (→
`cuda-qx-profiling-perf`).
...
3. **`OMP_NUM_THREADS=1` for chemistry benchmarks.** Required for
   ...
   explanation in `cuda-qx-solvers-chemistry` (Convention #1).
...
bars, statistical test). Get the DEM + shots from `cuda-qx-qec-decode`, then
run:

```python
# Assumes you already have `dem`, `shots`, `obs` from cuda-qx-qec-decode.
# (For how to build them, see cuda-qx-qec-decode/references/decode.md.)
import numpy as np
...
domain-specific setup delegated to `cuda-qx-solvers-algorithms` /
...
[ ] Git SHA of the cudaqx checkout reported.
...
4. If timing is unstable, run with `nsys` (`cuda-qx-profiling-perf`)
...
- Profiling for tail latency: `cuda-qx-profiling-perf`.
- Authoring a new method then benchmarking it: `cuda-qx-qec-extending`, `cuda-qx-solvers-extending`.
