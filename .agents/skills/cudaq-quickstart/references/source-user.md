...
CUDA-Q Libraries or recompile against a different CUDA-Q. If the user just
wants to *try* cudaq, see `pip-user.md` or `docker-user.md` first.
...
```bash
git clone --recursive https://github.com/NVIDIA/cudaq.git
cd cudaq
# Install CUDA-Q first if not already installed (default: $HOME/.cudaq)
# Build CUDA-Q Libraries:
mkdir -p build && cd build
cmake -G Ninja -S .. \
  -DCUDAQ_DIR=$HOME/.cudaq/lib/cmake/cudaq \
  -DCMAKE_INSTALL_PREFIX=$HOME/.cudaq \
...
If any of that fails, delegate to **`cudaq-build` SKILL.md** —
that skill owns build issues end-to-end.

After a successful build, set up the environment:

Set the install-path environment block from `cudaq-build/SKILL.md`
("Standard environment" section). The minimum two:

```bash
export CUDAQ_INSTALL_PREFIX=$HOME/.cudaq
export CUDAQX_INSTALL_PREFIX=$HOME/.cudaq
```

See `cudaq-build` for the full block (`CUDAQ_DIR`, `PATH`, `PYTHONPATH`).
...
`$HOME/.cudaq`) — see `cudaq-build/references/build.md`.
...
| Goal | Skill to open |
|------|---------------|
| Build issues, wheels, docs, containers | `cudaq-build` |
| QEC workflows (decode, custom code, real-time) | `cudaq-qec-decode` |
| Variational solvers (VQE, ADAPT, QAOA, GQE) | `cudaq-solvers-algorithms` |
| Add a new decoder or code | `cudaq-qec-extending` |
| Add a new operator pool / optimizer | `cudaq-solvers-extending` |
| Contribute a PR (and run/extend tests, CI) | `cudaq-contributing` |

## When stuck

1. `bash scripts/doctor.sh` for a structured environment dump.
2. `bash scripts/clean.sh` (when present) wipes `$HOME/.cudaq` and
   `build/` to start over.
3. `bash .agents/skills/_shared/scripts/preflight.sh --json` reports
   missing pieces in machine-readable form.
4. Open `cudaq-build` SKILL.md and walk the workflow index.
