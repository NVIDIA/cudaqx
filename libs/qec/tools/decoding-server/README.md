...
transport providers, per-decoder rings, or published endpoints. The existing
transport connection must remain live throughout the reload.
...
A restarted peer that presents a new QP, RKey, address, or transport session is
not discovered by `SIGHUP`; the server continues to hold the original endpoint
and must be restarted. Live transport reconnection requires a separate provider
lifecycle API and is outside decoder reconfiguration.

Decoder types, matrices, dimensions, and decoder custom arguments may change
while the decoder ID and dispatch shape remain stable, provided the encoded
request and response frames fit the preserved provider's slot capacity. The
runtime rejects an oversized frame safely, but
`QEC_DECODING_SERVER_CONFIG_APPLIED` confirms decoder construction and does
not independently guarantee end-to-end payload fit. Host decoders may also
