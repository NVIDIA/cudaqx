...
// Definition of the `memory_circuit` device kernel. This header is included by
// exactly one nvq++-compiled translation unit per binary: the offline copy that
// ships in `libcudaq-qec` (compiled without realtime decoding), and any
// application that wants a realtime-enqueueing copy. To enqueue syndromes to a
// realtime decoder, compile the including translation unit with
// `-DCUDAQ_QEC_ENABLE_REALTIME_DECODING` and link the appropriate decoding shim
// (see cudaq/qec/realtime/decoding.h). The offline `libcudaq-qec` build leaves
// the macro undefined so that it carries no reference to the decoding symbols.
...
#if defined(CUDAQ_QEC_ENABLE_REALTIME_DECODING)
...
__qpu__ void memory_circuit(const code::stabilizer_round &stabilizer_round,
                            ...
                            bool enqueue_syndromes, std::uint64_t decoder_id) {
#if !defined(CUDAQ_QEC_ENABLE_REALTIME_DECODING)
  // Realtime enqueueing is compiled out in this build.
  (void)enqueue_syndromes;
  (void)decoder_id;
#endif
  ...
  if (enqueue_syndromes)
    cudaq::qec::decoding::enqueue_syndromes(decoder_id, final_syndrome);
#endif
