...
Use `nsys` (`cuda-qx-profiling-perf`) to capture per-stage timings.
...
- For non-surface-code decoders, autonomous_decoder is the more
  flexible path: `cuda-qx-qec-realtime/references/autonomous-decoder.md`.
- For benchmarking many decoders side-by-side: `cuda-qx-benchmarking`.
- For profiling tail latency: `cuda-qx-profiling-perf`.
