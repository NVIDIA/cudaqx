...
the underlying neural network, see `cudaq-qec-ai-decoders` SKILL.md.
...
2. Profile with `nsys` (see `cudaq-profiling-perf`); the
   predecoder kernel and MWPM stage should each be visible as
   distinct ranges.
3. If retraining the predecoder, delegate to `cudaq-qec-ai-decoders`.
