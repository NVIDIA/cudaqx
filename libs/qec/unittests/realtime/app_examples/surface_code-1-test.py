    ...
    pytest.param(
        {
            "distance": 5,
            "p_cnot": 0.01,
            # HEAD used num_rounds=20 but decoded in decoder_window=10 windows;
            # the rewritten demo sizes the DEM by num_rounds directly, so the
            # full 20-round depth-2 multi_error_lut (480x4783) OOMs the module-
            # scoped in-process suite. 5 is the largest that fits here.
            "num_rounds": 5,
            "num_shots": 1000,
            "decoder_type": "multi_error_lut",
            "target": "quantinuum",
