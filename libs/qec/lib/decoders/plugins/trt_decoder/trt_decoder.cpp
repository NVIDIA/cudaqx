...
public:
  void log(Severity severity, const char *msg) noexcept override {
    ...
    if (severity >= Severity::kWARNING) {
      CUDAQX_INFO("[TensorRT] {}", msg);
    } else {
      CUDAQX_WARN("[TensorRT] {}", msg);
    }
  }
};
...
CaptureResult try_capture_cuda_graph(nvinfer1::IExecutionContext *context,
  ...
  try {
    ...
    CUDAQX_INFO("Attempting to capture CUDA graph during initialization...");
    ...
    CUDAQX_INFO("CUDA graph captured successfully during initialization");
    result.success = true;
...
trt_decoder::trt_decoder(const cudaq::qec::sparse_binary_matrix &H,
  ...
  try {
    ...
    CUDAQX_INFO("TensorRT engine I/O data types: input={}, output={}",
    ...
    CUDAQX_INFO("TensorRT model configuration: batch_size={}, "
    ...
    if (params.contains("use_cuda_graph")) {
      ...
      if (!use_cuda_graph) {
        CUDAQX_INFO("CUDA graphs explicitly disabled by user");
      }
    }

    // Check engine compatibility
    if (use_cuda_graph && !supports_cuda_graphs(impl_->engine.get())) {
      CUDAQX_WARN("Model has dynamic shapes or multiple profiles, "
    ...
    if (use_cuda_graph) {
      ...
      if (capture_result.success) {
        ...
        CUDAQX_INFO("TensorRT decoder initialized with CUDA graph execution");
      } else {
        CUDAQX_WARN(
      ...
      impl_->executor = TraditionalExecutor{};
      CUDAQX_INFO("TensorRT decoder initialized with traditional execution");
    }
    ...
    if (params.contains("global_decoder") &&
      ...
      if (!global_decoder_name.empty()) {
        ...
        CUDAQX_INFO("TensorRT decoder: global_decoder '{}' attached",
                    global_decoder_name);
      }
    }
    ...
    if (params.contains("O")) {
      ...
      CUDAQX_INFO("TensorRT decoder: decode_to_observables enabled "
                  "(num_observables={})",
                  num_observables_);
    }
    ...
    CUDAQX_WARN("TensorRT initialization failed: {}", e.what());
...
std::vector<decoder_result> trt_decoder::decode_batch_impl(
  ...
  try {
    ...
    if (log_residual_counts) {
      CUDAQX_INFO("TRT decoder: total non-zero input detectors = {}, total "
                  "non-zero residual detectors = {}",
                  total_input_nonzero, total_residual_nonzero);
    }

  } catch (const std::exception &e) {
    CUDAQX_WARN("TensorRT batch inference failed: {}", e.what());
...
void parse_precision(const std::string &precision,
  ...
  if (precision == "tf32") {
    ...
    CUDAQX_WARN("Precision '{}' is ignored when building strongly-typed "
                "networks. TensorRT uses the data types defined in the ONNX "
                "model. To use {}, export the model with the desired types.",
                precision, precision);
  } else {
    CUDAQX_WARN("Unknown precision '{}', using default (best)", precision);
  }
}
...
build_engine_from_onnx(const std::string &onnx_model_path,
  ...
  if (params.contains("batch_size") && network->getNbInputs() > 0) {
    ...
    if (dims.nbDims > 0 && dims.d[0] == -1) {
      ...
      CUDAQX_INFO("TensorRT optimization profile: batch min=1, opt=max={}",
                  batch_size);
    }
  }
...
void save_engine_to_file(nvinfer1::ICudaEngine *engine,
  ...
  CUDAQX_INFO("TensorRT engine saved to: {}", file_path);
}
