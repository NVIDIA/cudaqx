---
name: "cudaq-quickstart"
title: "CUDA-Q Libraries Quickstart and First-Time Setup"
description: >-
  First-time orientation for CUDA-Q Libraries. Use whenever a user is new to the
  repo, asks "how do I get started", "I just pip installed cudaq-qec what
  next", "hello world for cudaq", "what's the simplest example", or
  ...
  skills (cudaq-qec-decode, cudaq-solvers-algorithms, cudaq-build).
version: "0.1.2"
author: "CUDA-Q Libraries"
license: "Apache License 2.0"
compatibility: "Python 3.11+, Linux x86_64/aarch64 (pip), macOS (Docker only)"
tags: [cudaq, quickstart, onboarding, tutorial, hello-world, students, install]
...
# CUDA-Q Libraries Quickstart

The "you just discovered this repo, now what?" entry point. The three
workflow skills (`cudaq-build`, `cudaq-qec-decode`, `cudaq-solvers-algorithms`)
...
1. **Pip-only user** (student, researcher who wants to try cudaq):
   `pip install cudaq-qec` or `pip install cudaq-solvers`, run a Python
   example, read the published docs at <https://nvidia.github.io/cudaq/>.
   They cannot read local repo files.
2. **Docker user** (Mac, locked-down workstation, or "I want it to just
   work"): `docker run --gpus all -it ghcr.io/nvidia/cudaq`.
...
```
Q1: Has the user cloned the cudaq repo? (look for `libs/qec` in cwd)
...
`examples/` directory in the cudaq repo — only `docs/sphinx/examples/`.)
...
2. Where do I find more examples? (`docs/sphinx/examples/{qec,solvers}/`
   in source, or <https://nvidia.github.io/cudaq/> for pip users.)
3. Which workflow skill do I open next? (`cudaq-qec-decode`,
   `cudaq-solvers-algorithms`, or `cudaq-build` for build issues.)
...
|---------------------------------------------------|--------------------------------------------|
| Pick a decoder, run a circuit-level experiment    | `cudaq-qec-decode` SKILL.md                     |
| Tune VQE, ADAPT, QAOA, GQE, or chemistry          | `cudaq-solvers-algorithms` SKILL.md                 |
| Fix an install / build / docker / wheel issue     | `cudaq-build` SKILL.md                   |
| Write a new code or decoder                       | `cudaq-qec-extending` SKILL.md           |
| Write a new operator pool / optimizer             | `cudaq-solvers-extending` SKILL.md       |
| Set up real-time decoding on hardware             | `cudaq-qec-realtime` SKILL.md       |
