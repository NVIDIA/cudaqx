...
| Real-time C++ link errors on Quantinuum target                | Missing `-lcudaq-qec-realtime-decoding-quantinuum -Wl,--export-dynamic` | Add to nvq++ link line (see `cuda-qx-qec-realtime/references/hardware-helios.md`) |
...
| `OMP_NUM_THREADS` warning during chemistry tests              | Docs/build scripts pin `OMP_NUM_THREADS=1`                  | Expected for reproducibility — see `cuda-qx-solvers-chemistry` (Convention #1) |
