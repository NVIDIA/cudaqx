...
// Decoupled diagnostics: plain C, no cudaq dependency, so this header can be reused independently.
inline void affinity_warn(const std::string &msg) {
  std::fprintf(stderr, "[cudaq-qec affinity] WARNING: %s\n", msg.c_str());
}
// INFO/debug: only shown when CUDAQ_QEC_AFFINITY_DEBUG is set (a legitimate no-op like a
// single-node auto-derive must not spam; it is debug-loggable, not a failure).
...
inline void bind_this_thread_to_numa_node(int node,
  ...
  if (node >= static_cast<int>(sizeof(unsigned long) * 8))
    throw std::runtime_error(
        ...
        std::to_string(sizeof(unsigned long) * 8 - 1) + "); cannot bind memory policy");
  cpu_set_t prev;
  CPU_ZERO(&prev);
  ...
  if (syscall(SYS_set_mempolicy, mempolicy_syscall_mode(mode), &nodemask,
    ...
    affinity_warn("numa_node_id " + std::to_string(node) +
                  " requested but set_mempolicy failed: " + std::strerror(errno) +
                  "; memory not bound to node");
}
...
// Throws if node cannot be encoded in the mempolicy nodemask (node >= 64).
// Warns (does not throw) if the OS declines an otherwise well-formed request.
inline void bind_region_to_numa_node(void *p, std::size_t bytes, int node,
                                     mempolicy_mode mode = mempolicy_mode::preferred) {
  ...
  if (syscall(SYS_mbind, p, static_cast<unsigned long>(bytes),
              ...
              static_cast<unsigned long>(sizeof(nodemask) * 8), MPOL_MF_MOVE) != 0)
  ...
  for (int c : cores)
    if (c < 0 || c >= CPU_SETSIZE)
      throw std::invalid_argument("cpu_affinity core id " + std::to_string(c) +
                                  " is out of range [0, " + std::to_string(CPU_SETSIZE) + ")");
  cpu_set_t set;
  CPU_ZERO(&set);
  for (int c : cores)
    CPU_SET(c, &set);
...
inline void bind_this_thread_to_numa_node(int node,
  ...
  if (!warned) {
    affinity_warn("numa_node_id ignored: NUMA binding is only supported on Linux");
    warned = true;
  }
}
inline void bind_region_to_numa_node(void *, std::size_t, int,
                                     mempolicy_mode = mempolicy_mode::preferred) {}
