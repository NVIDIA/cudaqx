...
/// @brief Software FPGA emulator for QEC decode loop testing.
///
/// Emulates the FPGA's role in the QEC decode pipeline:
///   1. Hololink UDP control plane server (register read/write)
///   2. Playback BRAM (receives syndromes from FPGA playback tool)
///   3. RDMA transmit (sends syndromes to mock decoder bridge)
///   4. RDMA receive (receives corrections from bridge)
///   5. ILA capture RAM (stores corrections for verification readback)
///
/// Three-tool workflow:
...
struct EmulatorArgs {
  ...
  size_t page_size = 256;     // Default slot size for corrections RX
};
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    // RX buffer for incoming corrections (same page_size as bridge for
    // symmetry)
    ...
    // Post receive WQEs for corrections
    ...
    for (uint32_t window = 0; window < win_num && !g_shutdown; window++) {
        ...
        if (n > 0) {
          if (wc.status == IBV_WC_SUCCESS) {
            ...
            if (ila_armed) {
              ...
              uint8_t *corr_data = static_cast<uint8_t *>(rx_buffer.data()) +
                                   (rx_slot * args.page_size);
              store_ila_sample(regs, window, corr_data, wc.byte_len);
            }
      ...
      if ((window + 1) % 10 == 0 || window == win_num - 1) {
        std::cout << "  Window " << (window + 1) << "/" << win_num
                  << " (corrections: " << corrections_received
    ...
    std::cout << "  Corrections received: " << corrections_received << std::endl;
