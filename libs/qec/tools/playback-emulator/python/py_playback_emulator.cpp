...
/// `cpu_roce_endpoints` / `null_decoder_ids` is given (exactly one must be).
/// `sources` maps a schedule's source_id to a plain spec dict (see
/// `make_source`); a fresh syndrome_source is built from each spec for this
/// run alone.
run_result run_schedule(
  ...
  if (int(decoders.has_value()) + int(udp_endpoints.has_value()) +
          int(cpu_roce_endpoints.has_value()) +
          int(null_decoder_ids.has_value()) !=
      1)
    throw std::invalid_argument(
        "run: specify exactly one of decoders=, udp_endpoints=, "
  ...
  } else if (cpu_roce_endpoints) {
    owned_sessions = make_cpu_roce_sessions(
        *cpu_roce_endpoints, make_cpu_roce_options(cpu_roce_options_spec),
        cpu_roce_timeout_ms);
  } else {
