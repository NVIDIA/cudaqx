...
| `scripts/validation/container/validate_container.sh` | Validate published cudaqx container. |
...
| Example runs locally but fails in `test_examples.sh` | example writes outside its CWD or depends on `$HOME/.cudaqx` state |
