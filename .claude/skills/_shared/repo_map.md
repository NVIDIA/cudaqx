# CUDA-Q Libraries Repo Map

This file is shared across the `cudaq-qec-decode`, `cudaq-solvers-algorithms`, and
`cudaq-build` skills. It lives in `.agents/skills/_shared/` (the
...
| `cmake/`                           | Custom CMake modules (`include(CUDA-Q Libraries)`)                         |
...
| `CUDAQX_INSTALL_PREFIX`           | CUDA-Q Libraries install root (default `$HOME/.cudaq`)                       |
...
| `ghcr.io/nvidia/cudaq:latest`                   | Release container (built from `docker/release/Dockerfile`) |
