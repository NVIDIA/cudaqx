...
uccgsd::generate(const heterogeneous_map &config) const {
  ...
  auto addSingleExcitation = [](std::vector<cudaq::spin_op> &ops, std::size_t p,
                                std::size_t q) {
    if (p > q) {
  ...
  auto addDoubleExcitation = [](std::vector<cudaq::spin_op> &ops, std::size_t p,
    ...
    if (p > q && r > s) {
      ...
      cudaq::spin_op temp_op = c * cudaq::spin::y(s) * parity_b *
