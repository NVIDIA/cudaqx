...
# Verify the mirror got it
ls .claude/skills/cuda-qx-MYSKILL/
```

Then update `AGENTS.md` "Available skills" table.

```bash
git add .agents/skills/cuda-qx-MYSKILL AGENTS.md
git commit -s -m "skill: add cuda-qx-MYSKILL"
```
...
| Mirror has it but CI fails `--check` | committed a stale mirror; `.claude/` and `.cursor/` are gitignored, this should not happen unless gitignore was changed |
