...
The published container is the most reliable way to try cudaq on a
Mac, a locked-down workstation, or a fresh Linux box. Everything
(CUDA-Q, CUDA-Q Libraries, examples, notebooks) is preinstalled.

## Pull and run

```bash
docker pull ghcr.io/nvidia/cudaq
docker run --gpus all -it ghcr.io/nvidia/cudaq
```

On Mac (no NVIDIA GPU), omit `--gpus all`:

```bash
docker run -it ghcr.io/nvidia/cudaq
```
...
| Path | What |
|------|------|
| `/workspaces/cudaq/` | full source checkout |
| `/workspaces/cudaq/docs/sphinx/examples/` | curated examples |
...
(see `cudaq-build`); the user-facing release container is
`ghcr.io/nvidia/cudaq`.
...
Then delegate to `cudaq-qec-decode` SKILL.md.
...
```bash
docker run --gpus all -it -p 8888:8888 ghcr.io/nvidia/cudaq
# inside container
...
"Reopen in Container" feature. See `cudaq-build` SKILL.md for the
...
- ARM64 (Apple Silicon) images exist for the dev container; the user
  release image `ghcr.io/nvidia/cudaq` is x86_64-only at time of
